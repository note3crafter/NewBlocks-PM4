...
namespace TheNote\NewBlocks\entity\object;
...
class LightningBoltEntity extends Entity{
    ...
    public function sendImpactSound(Position $position, float $pitch = 1, float $volume = 1): void{
        $this->getWorld()->broadcastPacketToViewers($position, Main::playSound($position, "ambient.weather.lightning.impact", $pitch, $volume, true));

    }

    public function sendRoarSound(Position $position, float $pitch = 1, float $volume = 1): void{
        $this->getWorld()->broadcastPacketToViewers($position, Main::playSound($position, "ambient.weather.thunder", $pitch, $volume, true));
    }
}