...
class WeatheredCutCopper extends Opaque
{

    public function __construct(BlockIdentifier $idInfo, ?BlockBreakInfo $breakInfo = null)
    {
        parent::__construct($idInfo, "Weathered Cut Copper",$breakInfo ?? new BlockBreakInfo(3, BlockToolType::PICKAXE));
    }
    ...
    public function onInteract(Item $item, int $face, Vector3 $clickVector, ?Player $player = null) : bool{
        ...
        if($item instanceof $honey) {
            if ($player->getInventory()->getItemInHand()->getId() == 736) {
                $this->position->getWorld()->setBlock($this->position, BlockFactory::getInstance()->get(CustomIds::WAXED_WEATHERED_CUT_COPPER, 0));
